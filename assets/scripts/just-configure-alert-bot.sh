...
#  - room IDs         : written back into etc/config.yaml (network.matrix.*).
#
# Source of truth: etc/config.yaml `network.matrix.{admin,bot,warningsRoom,
# incidentsRoom}` and `network.domain`. The Matrix host is read from the
# generated network. Client API calls go to https://matrix.<domain>; the
# admin-only registration endpoint falls back to an SSH tunnel (as the nix
# deploy user) when it is not exposed publicly.
...
bot="$(yq -r '.network.matrix.bot // "alertbot"' "$cfg")"
...
# --- 4. Persist room IDs into config.yaml ----------------------------------
WARN_ID="$WARN_ID" yq -i '.network.matrix.warningsRoom = strenv(WARN_ID)' "$cfg"
INC_ID="$INC_ID" yq -i '.network.matrix.incidentsRoom = strenv(INC_ID)' "$cfg"
log "Room IDs written to etc/config.yaml (network.matrix.*)."

log "Done. Set 'darkone.service.monitoring.alerting.enable = true' on the"
log "monitoring host, then 'just generate' + 'just apply <host>'."
