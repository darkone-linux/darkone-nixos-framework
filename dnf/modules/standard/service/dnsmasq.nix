      ...
      settings = lib.mkMerge [
        {
          ...
          # Ne forward jamais le domaine headscale, répond NXDOMAIN immédiatement pour AAAA
          # -> Sinon le domaine serait résolu en adresse ipv4 tailnet interne et tailscale serait
          #    incapable de trouver headscale. Autre solution possible : filter-aaaa
          ++ (lib.optional isTailscaleSubnet "/${network.coordination.domain}.${network.domain}/")

          # Other zones (not current, not www)
          # -> TODO: supprimer ? Les adresses internes des zones sont déjà générées par le générateur,
          #          cette boucle ajoute les adresses ipv4 tailnet, pas utile...
