...
let
  ...
  cfg = config.darkone.security.kernelParams;
  isActive = dnfLib.mkIsActive (mainSecurityCfg // { inherit (cfg) enable; });
in
{
  options = {
    darkone.security.kernelParams.enable = lib.mkEnableOption "Active les paramètres noyau dynamiques ANSSI (R8–R14).";
  };

  config = lib.mkMerge [
    { darkone.security.kernelParams.enable = lib.mkDefault mainSecurityCfg.enable; }
