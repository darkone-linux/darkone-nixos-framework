...
in
{
  ...
  config = mkIf cfg.enable {
    ...
    environment = {
      ...
      etc."nixos".source = "${cfg.persistRootDir}/etc/nixos";

      # Persist root dir (btrfs+zstd)
      persistence."${cfg.persistRootDir}" = {
        hideMounts = true;
        directories = [
          "/var/log"
          "/etc/nixos"
      ...
      persistence."${cfg.persistRootDir}/databases" = {
        hideMounts = true;
